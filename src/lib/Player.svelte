<script>
    ...
    function updateFrame(dt) {
        ...
        if (playerEl) {
            playerEl.style.backgroundPositionX =
                -(frameIndex * FRAME_WIDTH) + "px";
            playerEl.style.backgroundPositionY =
                -(anim.row * FRAME_HEIGHT) + "px";
        }
    }
    ...
    onMount(() => {
        function gameLoop() {
            ...
            playerEl.style.transform = `translate(${x - size / 2}px, ${y - size / 2}px) scale(${scale})`;
...
<div
    bind:this={playerEl}
    class="absolute w-8 h-8 bg-no-repeat image-rendering-pixelated origin-center"
...
>
    {#if debug}
        <div
            class="absolute -top-12 left-0 text-xs bg-white px-1 border border-gray-300"
        >
